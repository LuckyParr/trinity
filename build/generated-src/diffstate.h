...
#define CONFIG_DIFF_STORE_WIDTH 1
typedef struct  {
  uint8_t  valid;
  uint64_t addr;
  uint64_t data;
...
#define CONFIG_DIFF_WB_VEC_WIDTH 32
typedef struct  {
  uint64_t data;
} DifftestVecWriteback;

#define CONFIG_DIFFTEST_FPWRITEBACK
#define CONFIG_DIFF_WB_FP_WIDTH 32
typedef struct  {
  uint64_t data;
} DifftestFpWriteback;
...
#define CONFIG_DIFFTEST_RUNAHEADEVENT
#define CONFIG_DIFF_RUNAHEAD_WIDTH 1
typedef struct  {
  uint8_t  valid;
  uint8_t  branch;
  uint8_t  may_replay;
...
#define CONFIG_DIFFTEST_RUNAHEADREDIRECTEVENT
typedef struct  {
  uint8_t  valid;
  uint64_t pc;
  uint64_t target_pc;
...
#define CONFIG_DIFF_L2TLB_WIDTH 1
typedef struct  {
  uint8_t  valid;
  uint8_t  valididx[8];
  uint64_t satp;
...
#define CONFIG_DIFF_RUNAHEAD_COMMIT_WIDTH 1
typedef struct  {
  uint8_t  valid;
  uint8_t  branch;
  uint8_t  may_replay;
...
#define CONFIG_DIFF_L1TLB_WIDTH 3
typedef struct  {
  uint8_t  valid;
  uint64_t satp;
  uint64_t vpn;
...
#define CONFIG_DIFF_WB_INT_WIDTH 32
typedef struct  {
  uint64_t data;
...
#define CONFIG_DIFF_LOAD_WIDTH 1
typedef struct  {
  uint8_t  valid;
  uint64_t paddr;
  uint8_t  opType;
...
typedef struct {
  ...
  DifftestFpWriteback            wb_fp[32];
  DifftestInstrCommit            commit[1];
  DifftestIntWriteback           wb_int[32];
  DifftestL1TLBEvent             l1tlb[3];
  DifftestL2TLBEvent             l2tlb[1];
  DifftestLoadEvent              load[1];
  DifftestLrScEvent              lrsc;
  DifftestRefillEvent            refill[3];
  DifftestRunaheadCommitEvent    runahead_commit[1];
  DifftestRunaheadEvent          runahead[1];
  DifftestRunaheadRedirectEvent  runahead_redirect;
  DifftestSbufferEvent           sbuffer[1];
  DifftestStoreEvent             store[1];
  DifftestTrapEvent              trap;
  DifftestTriggerCSRState        triggercsr;
  DifftestVecWriteback           wb_vec[32];
} DiffTestState;
